...
from shared_exchange_clients.trading.deribit.api_client import DeribitApiClient
from shared_utils.risk.models import MarginCalculationResult
...
class PortfolioMarginCalculator:
    ...
    async def calculate_margins(
        ...
        if not response.get("success"):
            ...
            log.error(error_msg)
            return MarginCalculationResult(
                initial_margin=0.0, maintenance_margin=0.0, is_valid=False, error_message=error_msg
            )
